...
def performSVNTuned(x_train, x_test, y_train, y_test):
    ...
    # model = svm.SVC(random_state=42,gamma=0.000001)
    # model = svm.SVC(random_state=42, gamma=0.000001)
    model = svm.SVC(random_state=42, C=1000.0)
    model.fit(x_train, y_train)
    y_pred = model.predict(x_test)
    ...
    '''viz = ValidationCurve(model, param_name='gamma',
                          param_range=[0.0000008, 0.0000009,0.000001,0.000002,0.000003], cv=10,
                          scoring='accuracy')'''
    viz = ValidationCurve(model, param_name='C',
                          param_range=[600,800,1000,1100,1200], cv=10,
                          scoring='accuracy')
